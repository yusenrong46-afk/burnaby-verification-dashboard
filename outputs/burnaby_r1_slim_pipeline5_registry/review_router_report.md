...
- `burnaby_r1_095` -> `rerun_with_evidence_bundle` / `medium`: Rerun using the evidence bundle; promote only if deterministic verifier passes. Check pipeline5_text_candidate_requires_review in page 5; evidence_id pipeline5_merged_rule_0090__api_01. Evidence bundle is safe to rerun through the verifier. Semantic match: burnaby_r1_045 (score 0.69, blockers different_numeric_value).
